...

{
	...
	// Check whether the input file can be open succesfully or not
	if (!in_file.good())
	{
	...
	// each line of the input file: movieTitle, actor1, actor2, ..., actorm
	// read one line at a time then tokenize it:
	// create a pair <movieTitle, the set of actors in the movie>
   ...
   while (true)
   { 
	cout << "This applicaton stores information and their Movies, please choose your option (Enter Q or q to quit):" << endl;
	...
	if (cin.fail())
	{
	    ...
	    cin.clear();
	// To clear the fail condition in cin
	    cin >> input_to_check;
...
void analyzeMovies(const IMDB& imdb)
{
     ...
     // Otherwise display all the actors in either of the two movies
	...
	if( opt == "A" || opt == "a")
{
	...
	for (auto i = actors_in_movie2.begin(); i != actors_in_movie2.end(); i++)
	    all.insert(*i);
	// Insert all the actors in movie 2 into a set with all the actors in movie 1
...
{	if(*i == *j)
	// Prints the actors that are in both movies
	cout << *i << endl;
...
{	set<string> all = imdb.find_actors_in_a_movie(matchedMovie1);
	set<string> actors_in_a_movie2 = imdb.find_actors_in_a_movie(matchedMovie2);
	set<string> notcommon;
	// notcommon is an empty set where the symmetric difference of all and actors_in_a_movie2 is placed
	set_symmetric_difference(all.begin(), all.end(), actors_in_a_movie2.begin(), actors_in_a_movie2.end(), inserter(notcommon, notcommon.begin()));
	cout << "Actors only in one of the two movies:" << endl;
	for(auto i = notcommon.begin(); i != notcommon.end(); i++)
{	// Prints out the actors in notcommon
	cout << *i << endl;
}
}
...
void analyzeCoActors(const IMDB& imdb)
{
	...
	coactors.erase(actor_name); 
	for (auto j = coactors.begin(); j != coactors.end(); j++)
	{
	     // Prints the actors that were in the movie with the given actor
	     cout << *j << endl;
	}
	cout <<"***********************************" << endl;
    }
